...
Bu doküman, `billing-service` mikroservisi için oluşturulacak GitHub Actions CI/CD ve GitOps iş akışlarını tanımlar.

---

## 1. İş Akışları Mimarisi

`.github/workflows/billing-service/` altında 5 temel iş akışı yer alır:

```text
.github/workflows/billing-service/
...
```yaml
name: Billing Service - CI Test

on:
  push:
    paths:
      - 'billing-service/**'
      - '.github/workflows/billing-service/01-ci-test.yml'
  pull_request:
    paths:
      - 'billing-service/**'
...
```yaml
name: Billing Service - Security Scan
  ...
  security-scan:
    ...
    steps:
      ...
      - name: Build Local Image
        run: docker build -t billing-service:scan ./billing-service
...
```yaml
name: Billing Service - K8s Manifest Validation

on:
  push:
    paths:
      - 'kubernetes/base/apps/billing-service/**'
  ...
  validate-manifests:
    ...
    steps:
      ...
      - name: Build Kustomize Manifests
        run: kustomize build kubernetes/base/apps/billing-service > /tmp/rendered.yaml
