...
Bu doküman, `doctor-service` mikroservisinin veritabanı şifreleri, JWT ortak anahtarı ve dahili iletişim token'ının HashiCorp Vault üzerinde güvenli olarak saklanması, erişim politikası (ACL Policy) ve Kubernetes Auth rolünün tanımlanması adımlarını içerir.

---

## 1. Vault Secret Yolları ve Anahtarları

`doctor-service` için aşağıdaki KV Version 2 secret yolları kullanılır:

| Secret Yolu (Path) | Anahtarlar (Keys) | Açıklama |
| :--- | :--- | :--- |
| `secret/data/hospital/doctor-service/db` | `username`, `password` | PostgreSQL `doctor_db` erişim kimlik bilgileri |
...
# 2. doctor-service ACL Politikasını oluştur
vault policy write doctor-service-policy - <<EOF
path \"secret/data/hospital/doctor-service/*\" {
  capabilities = [\"read\"]
}
...
- [x] `secret/hospital/doctor-service/db` secret'ının Vault'a başarıyla yazıldığı doğrulandı.
- [x] `doctor-service-policy` ACL politikasının oluşturulduğu teyit edildi.
- [x] `auth/kubernetes/role/doctor-service-role` rolünün `doctor-service-sa` ServiceAccount'ına bağlandığı doğrulandı.
