...
Bu doküman, `admission-service` mikroservisinin veritabanı şifreleri, JWT ortak anahtarı ve dahili iletişim token'ının HashiCorp Vault üzerinde güvenli olarak saklanması, erişim politikası (ACL Policy) ve Kubernetes Auth rolünün tanımlanması adımlarını içerir.

---

## 1. Vault Secret Yolları ve Anahtarları

`admission-service` için aşağıdaki KV Version 2 secret yolları kullanılır:

| Secret Yolu (Path) | Anahtarlar (Keys) | Açıklama |
| :--- | :--- | :--- |
| `secret/data/hospital/admission-service/db` | `username`, `password` | PostgreSQL `admission_db` erişim kimlik bilgileri |
...
# 2. admission-service ACL Politikasını oluştur
vault policy write admission-service-policy - <<EOF
path \"secret/data/hospital/admission-service/*\" {
  capabilities = [\"read\"]
}
...
- [x] `secret/hospital/admission-service/db` secret'ının Vault'a başarıyla yazıldığı doğrulandı.
- [x] `admission-service-policy` ACL politikasının oluşturulduğu teyit edildi.
- [x] `auth/kubernetes/role/admission-service-role` rolünün `admission-service-sa` ServiceAccount'ına bağlandığı doğrulandı.
