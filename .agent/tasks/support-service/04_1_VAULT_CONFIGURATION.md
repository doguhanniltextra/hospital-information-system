...
Bu doküman, `support-service` mikroservisinin veritabanı şifreleri, JWT ortak anahtarı ve dahili iletişim token'ının HashiCorp Vault üzerinde güvenli olarak saklanması, erişim politikası (ACL Policy) ve Kubernetes Auth rolünün tanımlanması adımlarını içerir.

---

## 1. Vault Secret Yolları ve Anahtarları

`support-service` için aşağıdaki KV Version 2 secret yolları kullanılır:

| Secret Yolu (Path) | Anahtarlar (Keys) | Açıklama |
| :--- | :--- | :--- |
| `secret/data/hospital/support-service/db` | `username`, `password` | PostgreSQL `support_db` erişim kimlik bilgileri |
...
# 2. support-service ACL Politikasını oluştur
vault policy write support-service-policy - <<EOF
path \"secret/data/hospital/support-service/*\" {
  capabilities = [\"read\"]
}
...
- [x] `secret/hospital/support-service/db` secret'ının Vault'a başarıyla yazıldığı doğrulandı.
- [x] `support-service-policy` ACL politikasının oluşturulduğu teyit edildi.
- [x] `auth/kubernetes/role/support-service-role` rolünün `support-service-sa` ServiceAccount'ına bağlandığı doğrulandı.
